"""Check the allowlisted Pages artifact; never publish repository-root exam files."""
import hashlib
import json
...
root_files = {'index.html', 'favicon.svg', 'sw.js', 'model-notice.txt', 'model-license.txt'}
for path in root.rglob('*'):
    ...
    allowed = (len(parts) == 1 and path.name in root_files or
               len(parts) == 2 and parts[0] == 'assets' and path.suffix in {'.js', '.css'} or
               len(parts) == 2 and parts[0] == 'runtime' and path.suffix in {'.wasm', '.mjs'} or
...
print(f'Artifact verified: only static app and public model assets; {size / 1e6:.1f} MB.')
